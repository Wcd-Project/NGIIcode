...
% I=imread('Yi.jpg','jpg');         %读取图像信息
% I=imread('../原始、加密、解密图片/51.jpg','jpg');         %读取图像信息
% I=imresize(I,0.55);
...
% xlabel('R通道随机点像素灰度值');ylabel('与该点相邻水平方向像素灰度值');title('原始图像R通道水平相关性曲线');
figure;scatter(XX_G_SP,YY_G_SP,18,'filled');
% xlabel('G通道随机点像素灰度值');ylabel('与该点相邻水平方向像素灰度值');title('原始图像G通道水平相关性曲线');
figure;scatter(XX_B_SP,YY_B_SP,18,'filled');
% xlabel('B通道随机点像素灰度值');ylabel('与该点相邻水平方向像素灰度值');title('原始图像B通道水平相关性曲线');
%垂直
figure;scatter(XX_R_CZ,YY_R_CZ,18,'filled');
% xlabel('R通道随机点像素灰度值');ylabel('与该点相邻垂直方向像素灰度值');title('原始图像R通道垂直相关性曲线');
figure;scatter(XX_G_CZ,YY_G_CZ,18,'filled');
% xlabel('G通道随机点像素灰度值');ylabel('与该点相邻垂直方向像素灰度值');title('原始图像G通道垂直相关性曲线');
figure;scatter(XX_B_CZ,YY_B_CZ,18,'filled');
% xlabel('B通道随机点像素灰度值');ylabel('与该点相邻垂直方向像素灰度值');title('原始图像B通道垂直相关性曲线');
%对角线
figure;scatter(XX_R_DJX,YY_R_DJX,18,'filled');
% xlabel('R通道随机点像素灰度值');ylabel('与该点相邻对角线方向像素灰度值');title('原始图像R通道对角线相关性曲线');
figure;scatter(XX_G_DJX,YY_G_DJX,18,'filled');
% xlabel('G通道随机点像素灰度值');ylabel('与该点相邻对角线方向像素灰度值');title('原始图像G通道对角线相关性曲线');
figure;scatter(XX_B_DJX,YY_B_DJX,18,'filled');
% xlabel('B通道随机点像素灰度值');ylabel('与该点相邻对角线方向像素灰度值');title('原始图像B通道对角线相关性曲线');
%R通道
...
for i=2:r
    Q1_R=DNA_code(fenkuai(t,I1,i),X(i));   %对原始图像R通道每一个分块按X对应的序号进行DNA编码
    Q1_G=DNA_code(fenkuai(t,I2,i),X(i));
    Q1_B=DNA_code(fenkuai(t,I3,i),X(i));
    
    Q2=DNA_code(fenkuai(t,R,i),Y(i));   %对R的每一个分块按Y对应的序号进行DNA编码
    %R通道
    Q3_R=DNA_operation(Q1_R,Q2,Z(i));         %对上面两个编码好的块按Z对应的序号进行DNA运算
    Q4_R=DNA_operation(Q3_R,Q_last_R,Z(i));     %运算结果在和前一块按Z对应的序号再一次进行运算，称为扩散
    Q_last_R=Q4_R;
    %G通道
    ...
    Q_R((xx-1)*t+1:xx*t,(yy-1)*t+1:yy*t)=DNA_decode(Q4_R,H(i));    %将每一块合并成完整的图Q
...
I_new=Q_jiami;
% imwrite(Q_jiami,'加密后的lena.png','png');     
imwrite(Q_jiami,'加密后的Yi.png','png'); 
% imwrite(Q_jiami,'加密后的51.png','png');   
...
%水平
figure;scatter(XX_R_SP,YY_R_SP,18,'filled');
% xlabel('R通道随机点像素灰度值');ylabel('与该点相邻水平方向像素灰度值');title('加密图像R通道水平相关性曲线');
figure;scatter(XX_G_SP,YY_G_SP,18,'filled');
% xlabel('G通道随机点像素灰度值');ylabel('与该点相邻水平方向像素灰度值');title('加密图像G通道水平相关性曲线');
figure;scatter(XX_B_SP,YY_B_SP,18,'filled');
% xlabel('B通道随机点像素灰度值');ylabel('与该点相邻水平方向像素灰度值');title('加密图像B通道水平相关性曲线');
%垂直
figure;scatter(XX_R_CZ,YY_R_CZ,18,'filled');
% xlabel('R通道随机点像素灰度值');ylabel('与该点相邻垂直方向像素灰度值');title('加密图像R通道垂直相关性曲线');
figure;scatter(XX_G_CZ,YY_G_CZ,18,'filled');
% xlabel('G通道随机点像素灰度值');ylabel('与该点相邻垂直方向像素灰度值');title('加密图像G通道垂直相关性曲线');
figure;scatter(XX_B_CZ,YY_B_CZ,18,'filled');
% xlabel('B通道随机点像素灰度值');ylabel('与该点相邻垂直方向像素灰度值');title('加密图像B通道垂直相关性曲线');
%对角线
figure;scatter(XX_R_DJX,YY_R_DJX,18,'filled');
% xlabel('R通道随机点像素灰度值');ylabel('与该点相邻对角线方向像素灰度值');title('加密图像R通道对角线相关性曲线');
figure;scatter(XX_G_DJX,YY_G_DJX,18,'filled');
% xlabel('G通道随机点像素灰度值');ylabel('与该点相邻对角线方向像素灰度值');title('加密图像G通道对角线相关性曲线');
figure;scatter(XX_B_DJX,YY_B_DJX,18,'filled');
% xlabel('B通道随机点像素灰度值');ylabel('与该点相邻对角线方向像素灰度值');title('加密图像B通道对角线相关性曲线');
%R通道
Q_R=double(Q_R);
